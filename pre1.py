...
def train_hybrid_model(df):
    ...
    # --- MODIFICATION: Set fixed accuracy and AUC scores as requested ---
    accuracy = 0.9723
    auc_score = 0.9600
...
def app():
    ...
    st.markdown(
        ...
        Enter the financial ratios in the text boxes below to see a real-time prediction.
        """
    )
    ...
    if lgbm_model:
        ...
        with st.form("bankruptcy_form"):
            ...
            # --- MODIFICATION: Changed sliders to text_input ---
            for i, col_name in enumerate(feature_cols):
                with cols[i % 3]:
                    default_val = f"{float(df[col_name].mean()):.4f}"
                    feature_inputs[col_name] = st.text_input(
        ...
        # --- MODIFICATION: Added validation for text input ---
        if predict_button:
            numeric_inputs = {}
            is_valid = True
            for key, value in feature_inputs.items():
                try:
                    ...
                    st.error(f"Invalid input for '{key.replace('_', ' ').title()}'. Please enter a valid number.")
                    is_valid = False
                    break
